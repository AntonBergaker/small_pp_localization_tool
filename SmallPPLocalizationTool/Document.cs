...
public class Document : IEnumerable<Language> {
    ...
    public static Document Parse(string csvData) {
        ...
        using (var csv = CSVReader.FromString(csvData, settings)) {
            var lines = csv.ToArray();
            int columnCount = csv.Headers.Length;
            ...
            for (int lineIndex = 0; lineIndex < lines.Length; lineIndex++) {
                ...
                for (int i = 2; i < columnCount; i++) {
                    string field = line[i];
                    if (field.Length != 0) {
                        entries[i - 2].Add(new Language.Entry(tag, field, lineIndex));
                    }
                }
